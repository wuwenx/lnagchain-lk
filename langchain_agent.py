...
def _run_agent(
    ...
    if image_bytes_list and not VISION_MULTIMODAL:
        ...
        system_content = (
            ...
            + str(n)
            + " 张图，可能含聊天图片或飞书文档内嵌图）。若用户只发了「读取」「看图」等简短文字并依赖截图，请明确说明你**看不到图**，并建议：① 用文字描述界面或需求要点；② 接口细节请通过项目 **Apifox** 或 `/api` 查询；③ 或将 OPENAI_API_BASE 换为支持 OpenAI 多模态 vision 的端点（如官方 gpt-4o 等），并设置 VISION_MULTIMODAL=true。若下方【文档内容】里有文字，仍可仅基于文字回答。"
        )
